...
public class StylusTip : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        UnityEngine.Pose T_device = new UnityEngine.Pose(OVRInput.GetLocalControllerPosition(m_controller), OVRInput.GetLocalControllerRotation(m_controller));
        UnityEngine.Pose T_world_device = T_device.GetTransformedBy(m_trackingSpace);
        UnityEngine.Pose T_world_stylusTip = GetT_Device_StylusTip(m_controller).GetTransformedBy(T_world_device);
    ...
    private static UnityEngine.Pose GetT_Device_StylusTip(OVRInput.Controller controller) {
        ...
        UnityEngine.Pose T_device_stylusTip = UnityEngine.Pose.identity;

        if (controller == OVRInput.Controller.LTouch || controller == OVRInput.Controller.RTouch) {
            T_device_stylusTip = new UnityEngine.Pose(
